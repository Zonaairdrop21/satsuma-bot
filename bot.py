...
class SatsumaBot:
    ...
    async def convert_to_vesuma(self, private_key, amount, lock_time_days):
        try:
            ...
            # --- STEP 2: Call create_lock on the veSUMA contract ---
            log.processing("Sending veSUMA create_lock transaction...")
            vesuma_contract = self.w3.eth.contract(address=self.config["vesuma_address"], abi=VESUMA_ABI)
            
            create_lock_tx = vesuma_contract.functions.create_lock(amount_wei, unlock_time).build_transaction({
                "from": account.address,
                "gas": 400000,
                "gasPrice": self.w3.eth.gas_price,
                "nonce": nonce
