...
# The ABI for the `lock` function based on your input
VESUMA_ABI = [
    {
        "name": "lock",
        "inputs": [
...
class SatsumaBot:
    ...
    async def convert_to_vesuma(self, private_key, amount, lock_time_days):
        try:
            ...
            # lockDuration is in seconds
            lock_duration = lock_time_days * 24 * 60 * 60
            nonce = self.w3.eth.get_transaction_count(account.address)
            ...
            # --- STEP 2: Send the lock transaction using the new ABI ---
            log.processing("Sending veSUMA conversion (lock) transaction...")
            
            vesuma_contract = self.w3.eth.contract(address=self.config["vesuma_address"], abi=VESUMA_ABI)
            
            lock_tx = vesuma_contract.functions.lock(
                amount_wei,
                lock_duration
            ).build_transaction({
                "from": account.address,
